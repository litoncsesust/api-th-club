...
<div class="row justify-content-center color-bg-light n-margin">
	<div class="pt-5 pb-3 col-sm-10 col-md-10">
		<h4>{{ __('OPRET PRODUKT') }}</h4>
		<a class="btn btn-secondary btn-sm" href="{{ route('product.index') }}">Tilbage til produkt List</a>
		<hr>
		{!! Form::open(['method' => 'POST', 'route' => ['product.store'],'class' => '', 'enctype'=>'multipart/form-data']) !!}
		<div id="cc" class="row">
			<div class="col">
				<!-- Category Field -->
		...
		<div class="row">
			<div class="col">
				...
				<label class="form-check-label switch mb-3" for="is-featured">
					<input type="checkbox" name="isFeatured" value="0" id="is-featured">
					<span class="slider round"></span>Featured produkt
				</label>
				<div class="p-3 mb-3 color-bg-light relative">
				<p class="condensed">UPLOAD BILLEDE</span>
				<!-- Image Field -->
				<div class="uploader {{ $errors->has('product_img[]') ? ' has-error' : '' }}">
					<input type="file" class="custom-file-input" name="product_img[]" id="exampleInputFile" aria-describedby="fileHelp">
					<label class="custom-file-label" for="exampleInputFile">Vælg fil</label>
					<span class="help-block text-danger">
			...
			<div class="cp-form-label-group {{ $errors->has('total_number') ? ' has-error' : '' }}">
				...
				<!-- {!! Form::number('initial_point', null, ['class' => 'cp-form-control', 'placeholder' => 'ANGIV POINT']) !!} -->
				{!! Form::select('initial_point', $points, ['class' => 'cp-form-control']) !!}
				<!-- {!! Form::label('initial_point', 'ANGIV POINT', ['class' => 'control-label']) !!} -->
				<span class="help-block text-danger">
